<script>
  ...
  let themeOptions = $derived(
    themes.map(t => ({ id: t.id_theme, label: t.theme }))
  );

  let supportOptions = $derived(
    supports.map(s => ({ id: s.id_support, label: s.support }))
  );
...
<div class="d-flex justify-content-between align-items-center mb-3">
  <div class="text-center">
    <h3 class="text-xl font-semibold">🎬 Gestión de Películas </h3>
  </div>
