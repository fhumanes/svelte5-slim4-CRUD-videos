...
// Función para cancelar la eliminación de una película
  ...
  let fieldFilter = $derived([
            ...
            { id: "price", type: "number", label: "Precio" , placeholder: "Buscar por Precio, igual o mayor", filter: "greaterOrEqual" },
            { id: "startDate", type: "date", label: "Fecha", placeholder: "Buscar por Fecha, igual o mayor", filter: "greaterOrEqual" },
            { id: "rating",  type: "text", label: "Valoración", placeholder: "Buscar por Valoración" },
            { id: "theme", type: "text", label: "Tema", options: getOptions(movies, "theme"), placeholder: "Buscar por Tema" },
            { id: "support",  type: "text", label: "Soporte", options: getOptions(movies, "support"), placeholder: "Buscar por Soporte" }
          ]);
...
<Toasts />
  <div class="block justify-between items-center mb-4 ">
    <h2 class="text-xl font-semibold">🎬 Películas con Filtros y Ordenación</h2>
    <br/>
   <!-- Definición de los filtros Externos -->
  <div  class="block justify-between items-center">
    <span class="font-semibold">🔍 Aplicar Filtros:</span>
    <Tabs value={filterId} options={filterTabs} onchange={handleFilterChange} />

    <div class="bg-base-200 rounded-lg max-w-155 mb-4 mt-4 p-1">
  ...
  <div class="flex gap-2">
        <button class="btn  btn-primary" onclick={clearFilters}>🧹 Borrar filtros</button>
        ...
        <div class="tooltip tooltip-right tooltip-info" data-tip="Cliqueame para exportar los datos a un Excel">
...
{:else}
  <div class="border-2 border-primary shadow-xl shadow-gray-400  w-246  pb-1">
    <ContextMenu
