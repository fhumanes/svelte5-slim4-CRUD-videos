...
// =========================
// Editor Items (solo 1 campo)
// =========================
registerEditorItem("text", Text);

let editorItems = [
  { key: "theme", label: "Tema", comp: "text", required: true, maxLength: 50 }
];
...
async function fetchAllRecords() {
  try {
    ...
    // Normalizamos id_theme → id
    records = array.map(({ id_theme: id, ...rest }) => ({
      id,
      ...rest
    }));
...
function updateSelected() {
  // No lo necesitamos aquí, pero lo dejo por consistencia
}
...
<div class="flex justify-between items-center mb-4 ">
  <h2 class="text-xl font-semibold">Gestión de Temas</h2>
