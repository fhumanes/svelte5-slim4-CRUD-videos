<script>
  ...
  let editorItems = $derived([
    { key: "support", label: "Soporte", comp: "text", required: true, maxLength: 30 }
  ]);

  async function fetchAllSupports() {
    try {
      const res = await api.get('/supports');
      supports = res.data.map(s => ({
        id: s.id_support,
        support: s.support
      }));
      /*
      addToast({
        message:'Soportes cargados correctamente' ,
        type: 'success',
        dismissible: true,
        timeout: 2000
      });
      */
      ...
      addToast({
        message: 'No se pudieron cargar los soportes. Por favor, intente de nuevo más tarde.',
        type: 'error',
        dismissible: true,
        timeout: 0
      });
  ...
  function transformSupportPayload(values) {
    const { support } = values;
    return { support };
  }

  async function handleEditorAction({ item, values }) {
    const payload = transformSupportPayload(values);
    try {
      if (item.id === "save") {
        if (editorMode === "add") {
  ...
  async function proceedDelete() {
    try {
      ...
      supports = supports.filter(s => s.id !== itemToDelete.id);
      addToast({
  ...
  function handleContext(ev) {
    ...
    switch (ev.action?.id) {
      ...
      case "delete":
        if (id) {
          const supportItem = supports.find(s => s.id === id);
          if (supportItem) {
            confirmDelete(supportItem);
          }
        }
        break;
      default:
...
<div class="d-flex justify-content-between align-items-center mb-3">
  ...
  <div class="d-flex gap-2">
    ...
    <Button type="primary" text="➕ Agregar soporte" onclick={() => {
      editorMode = "add";
      editorValues = {};
      showEditor = true;
    }} />
